...
## [1.2.4] — 极限场景：双场景对话重置 + 失败可见提示 + 测试覆盖

- **场景 1（全新对话首条消息）**：撤回/编辑首条消息 → 归档整个对话 + **空白新对话顶替**（打错秒撤回场景，无需 fork）
- **场景 2（任何"说一半截断"状态：回合未闭合 turn-open / 首条无边界 no-boundary）**：自动删半截对话 → **恢复并打开父版本**（上一次模型回复处）重新开始（家族内）；无血缘降级空白顶替；编辑文本经 resume 机制带到新起点自动重发
- **失败可见提示**：no-boundary / turn-open 等失败显示三语原因（5s 消失），不再点了没反应
