    ...
    function UserBubbleView(props) {
      ...
      async function confirmEdit() {
        ...
        try {
          ...
          var attachRefs = (cachedEditMsg[myKey] && cachedEditMsg[myKey].attachRefs) ? cachedEditMsg[myKey].attachRefs.map(function(r){ return { attachmentId: r.attachmentId, mediaType: r.mediaType, name: r.name }; }) : [];
          // 桥接：旧会话 live 时把消息图片取成字节注册进官方单例（新会话 addImages 直接用）
          var bridgedIds = await bridgeSessionImages(sid, attachRefs);
          setEditing(false);
          ...
          try { localStorage.setItem("dsh-easyrewrite:resume-send:" + newId, JSON.stringify({ draftText: newText, t: Date.now(), imageIds: bridgedIds })); } catch (e) { /* ignore */ }
          try {
      ...
      if (editing) {
        ...
        renderMessageImagesCompat(editImgs, props),
          React.createElement(
