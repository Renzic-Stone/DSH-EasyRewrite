...
// ---------- 滚动锚定：已交还官方 chatScrollPositions（按会话原生保存/恢复滚动位，review #4 删自建轮子） ----------
    ...
    var SETTINGS_I18N = {
      zh: {
        ...
        insertImageShort: "拖入此处添加图片至正在编辑的消息",
      ...
      en: {
        ...
        insertImageShort: "Drop image here to add to the message being edited",
      ...
      ja: {
        ...
        insertImageShort: "ここにドロップで編集中のメッセージに画像を追加",
    ...
    function UserBubbleView(props) {
      ...
      var sDragActive = React.useState(false); // 文件拖入窗口中（编辑态虚线框dropzone显隐）
      var dragActive = sDragActive[0];
      var setDragActive = sDragActive[1];
      ...
      // 编辑态：document 捕获级接管文件拖拽（bug①修复 + 拖入虚线框dropzone）
      // dragenter/dragover/drop 全部捕获级拦截 → 官方全屏提示层不出现。
      // dropzone 虚线框渲染在编辑卡片内部（图片预览区域位置），非全屏覆盖。
      React.useEffect(function () {
        if (!editing) return;
        ...
        function onDragEnter(e) { if (!looksLikeFileDrag(e)) return; suppress(e); setDragActive(true); }
        function onDragOver(e) { if (!looksLikeFileDrag(e)) return; suppress(e); }
        function onDrop(e) {
          suppress(e);
          setDragActive(false);
          try {
      ...
      if (editing) {
        ...
        dragActive ? React.createElement("div", { key: "dropzone", style: { minHeight: "88px", border: "3px dashed var(--dsw-static-deepseek-500, #4d6bfe)", borderRadius: "16px", background: "rgba(77,107,254,0.06)", display: "flex", alignItems: "center", justifyContent: "center", padding: "12px", marginBottom: "6px" } },
            React.createElement("span", { style: { fontSize: "14px", color: "var(--dsw-static-deepseek-500, #4d6bfe)", textAlign: "center" } }, L.insertImageShort)
          ) : null,
        React.createElement("div", { style: { position: "relative", marginBottom: "6px" } },
          dragActive ? React.createElement("div", { style: { position: "absolute", inset: "0", zIndex: 5, minHeight: "72px", border: "3px dashed var(--dsw-static-deepseek-500, #4d6bfe)", borderRadius: "16px", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", background: "rgba(0,0,0,0.15)", display: "flex", alignItems: "center", justifyContent: "center" } },
            React.createElement("span", { style: { fontSize: "14px", color: "var(--dsw-static-deepseek-500, #4d6bfe)", textAlign: "center", padding: "0 16px" } }, L.insertImageShort)
          ) : null,
