...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function editWidthCustom() {
      try { var n = parseInt(localStorage.getItem("dsh-easyrewrite:editWidthCustom") || "", 10); return isFinite(n) && n > 100 ? n : 360; } catch (e) { return 360; }
    }
    ...
    function EasyRewriteSettingsCard() {
      ...
      var radioStyle = { display: "flex", alignItems: "center", gap: "6px", fontSize: "13px", color: "var(--dsw-alias-label-secondary)", cursor: "pointer" };
      var inputStyle = {
        width: "90px",
      ...
      function radioGroup(name, options, value, onChange) {
        return React.createElement("div", { style: groupStyle },
          options.map(function (opt) {
            return React.createElement("label", { key: opt[0], style: radioStyle },
              React.createElement("input", {
                type: "radio", name: name, value: opt[0],
                checked: value === opt[0],
                onChange: function () { onChange(opt[0]); }
              }),
              opt[1]
            );
          })
                ...
                : actionButton("撤回", "撤回", function (e) {
                    ...
                    if (recallConfirmEnabled()) {
                      ...
                      writePending(sessionId, {
                        ...
                        visualHide: vMode === "minimal", updatedAt: Date.now()
                      });
