...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    async     function rebuildDraftAttachments(attachmentRefs, props, sessionId, recordPending) {
      ...
      try {
        ...
        for (var i = 0; i < attachmentRefs.length; i++) {
          ...
          try {
            // 缓存优先：先读内存缓存（归档前预取的 blob），有就直接用（不依赖新会话 resolveImage）
            var cached = getCachedAttachment(ref.attachmentId);
            if (cached && cached.blob) {
              var cblob = cached.blob;
              var cname = (ref.name && typeof ref.name === "string" && ref.name) ? ref.name : (cached.name || "attachment." + (cached.mediaType || "img"));
              files.push(new File([cblob], cname, { type: cached.mediaType || cblob.type || "application/octet-stream" }));
              log("info", "attach", "命中缓存附件", { attachmentId: ref.attachmentId, size: cblob.size, type: cached.mediaType });
              continue;
            }
            log("info", "attach", "缓存未命中，走 resolveImage 拉取", { i: i, attachmentId: ref.attachmentId, mediaType: ref.mediaType });
            var url = null;
            ...
            // 写缓存，供后续重发复用
            putCachedAttachment(ref.attachmentId, blob, ref.mediaType || blob.type, name);
    ...
    // ---------- 附件内存缓存：动作前（归档前）把图片 blob 抓到本地，重发时直接用 ----------
    // 用户方案：选中消息时（旧会话还 live）取 blob；重发时用本地 blob 发出，然后才归档。避免归档后读不到。
    var attachMemCache = {}; // attachmentId -> { blob, mediaType, name }
    var pendingAttachIds = []; // 撤回确认时重建到输入框的图 id（× 取消时需移除）
    var attachCacheListeners = [];
    function onAttachCached(fn) { attachCacheListeners.push(fn); }
    function notifyAttachCached() { for (var i = 0; i < attachCacheListeners.length; i++) { try { attachCacheListeners[i](); } catch (e) { /* ignore */ } } }
    function putCachedAttachment(attachmentId, blob, mediaType, name) {
      if (!attachmentId || !blob) return;
      attachMemCache[attachmentId] = { blob: blob, mediaType: mediaType || blob.type, name: name, ts: Date.now() };
      notifyAttachCached();
    }
    function getCachedAttachment(attachmentId) {
      var e = attachMemCache[attachmentId];
      return e ? e : null;
    }
    // 从 DOM 收集当前输入框附件区的待发图片（用户撤回后手动添加的图）：扫描 blob: 图片并 fetch 数据
    // 官方输入框附件区：data-slot="conversation.input.attachments" 内的 img[src^="blob:"]
    function collectComposerImages() {
      var out = [];
      try {
        var slotEl = document.querySelector('[data-slot="conversation.input.attachments"]');
        if (!slotEl) return out;
        var imgs = slotEl.querySelectorAll('img[src^="blob:"]');
        imgs.forEach(function (im) {
          try {
            var src = im.getAttribute("src");
            var alt = im.getAttribute("alt") || ("attachment.png");
            if (src && src.indexOf("blob:") === 0) {
              out.push({ src: src, name: alt });
            }
          } catch (e) { /* ignore */ }
        });
      } catch (e) { /* ignore */ }
    ...
    // 把 DOM 收集的 blob 图 fetch 成 File 并写入内存缓存（attachmentId 用临时生成）
    async function absorbComposerImages(composerImgs) {
      var refs = [];
      if (!composerImgs || composerImgs.length === 0) return refs;
      for (var i = 0; i < composerImgs.length; i++) {
        try {
          var ci = composerImgs[i];
          var resp = await fetch(ci.src);
          if (!resp.ok) continue;
          var blob = await resp.blob();
          var tmpId = "composer-" + Date.now() + "-" + i;
          putCachedAttachment(tmpId, blob, blob.type, ci.name);
          refs.push({ attachmentId: tmpId, mediaType: blob.type || "image/png", name: ci.name });
          log("info", "attach", "收集输入框手动添加图片", { name: ci.name, size: blob.size, type: blob.type });
        } catch (e) { log("warn", "attach", "收集输入框图片失败", { err: String(e && e.message ? e.message : e) }); }
      }
      return refs;
    }
    ...
    function RecallBanner(props) {
      ...
      React.useEffect(function () {
        ...
        bar.style.cssText = "display:flex;align-items:center;justify-content:space-between;margin:0 16px;border-bottom:1px solid var(--dsw-alias-border-l2,rgba(128,128,128,0.25));padding:2px 6px 12px;";
        ...
        xBtn.addEventListener("click", function (e) {
          ...
          // 取消撤回：移除撤回确认时重建到输入框的图片（× 后恢复原样）
          if (ia && typeof ia.removeImage === "function" && pendingAttachIds.length > 0) {
            for (var pxi = 0; pxi < pendingAttachIds.length; pxi++) {
              try { ia.removeImage(pendingAttachIds[pxi]); } catch (e) { /* ignore */ }
            }
            pendingAttachIds = [];
          }
      ...
      async function doRecallThenSend(p) {
        ...
        try {
          ...
          // 0) 撤回前收集：只以 DOM 附件区现状为准（用户删除的图 DOM 已无，不会带上）
          // 撤回确认时已把原图重建到输入框（DOM 可见）；用户可删可改；此处收集 DOM 最终态
          var recallAtts = [];
          try {
            var composerImgs = collectComposerImages();
            var composerRefs = await absorbComposerImages(composerImgs);
            for (var rc = 0; rc < composerRefs.length; rc++) recallAtts.push(composerRefs[rc]);
            log("info", "attach", "撤回收集完成（仅 DOM 现状）", { count: recallAtts.length, domImgs: composerImgs.length });
          } catch (e) { log("warn", "attach", "撤回收集图片附件失败", { err: String(e && e.message ? e.message : e) }); }
          ...
          try { localStorage.setItem("dsh-easyrewrite:resume-send:" + newId, JSON.stringify({ draftText: sendText, t: Date.now(), attachments: recallAtts })); } catch (e) { /* ignore */ }
          // 2) 无痕替换：归档原会话 → 打开新会话
          var archived = false;
          try {
    ...
    // 渲染消息图片缩略图（问题1 修复：覆盖 chat.node 后官方图片槽不再自动注入，需自行渲染）
    function renderMessageImagesCompat(images, props) {
      if (!images || images.length === 0) return null;
      if (props && typeof props.renderMessageImages === "function") {
        try { return props.renderMessageImages({ images: images, align: "end" }); } catch (e) { /* 退化 */ }
      }
      if (props && typeof props.loadImage !== "function") return null;
      return React.createElement(
        "div", { style: { display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "6px", marginBottom: "6px" }, "data-dsh-easyrewrite": "images" },
        images.map(function (item) {
          return React.createElement(MessageImageCompat, { key: item.attachment && item.attachment.attachmentId, attachment: item.attachment, loadImage: props.loadImage, sessionId: props.sessionId });
        })
      );
    }

    // 自渲染图片组件：loadImage(attachment) → URL → <img>
    function MessageImageCompat(props) {
      ...
      React.useEffect(function () {
        ...
        try {
          if (typeof props.loadImage === "function") {
            props.loadImage(props.attachment).then(function (u) { if (!dead) setUrl(u); }).catch(function (e) { /* ignore */ });
          }
        } catch (e) { /* ignore */ }
      ...
      return url ? React.createElement("img", { src: url, style: style, alt: "attachment" }) : null;
    }
