...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    /** 编辑宽度三档（localStorage dsh-easyrewrite:editWidth；默认 wrap 自然换行 ≈748px）。 */
    function editWidthStyle() {
      var w = "wrap";
      try { w = localStorage.getItem("dsh-easyrewrite:editWidth") || "wrap"; } catch (e) { /* ignore */ }
      if (w === "bubble") return { maxWidth: 360 };
      if (w === "composer") return { maxWidth: "100%" };
      return { maxWidth: "var(--dsh-chat-content-width, 748px)" };
    }
    ...
    function UserBubbleView(props) {
      ...
      function enterEdit() {
        ...
        setEditing(true);
        setEditText(text);
        log("info", "edit", "进入编辑态", { sessionId: sessionId, targetSeq: realSeq });
      }
      function cancelEdit() {
