...
// ---------- 滚动锚定：已交还官方 chatScrollPositions（按会话原生保存/恢复滚动位，review #4 删自建轮子） ----------
    ...
    function RecallBanner(props) {
      ...
      React.useEffect(function () {
        ...
        function placeBar() {
          try {
            ...
            if (hasImages && sEl) {
              ...
              bar.style.top = Math.max(0, sr.top - bar.offsetHeight - 2) + "px";
              ...
              // 不带图：card 内部、文本输入区正上方（文档流内，含分界线）
        ...
        placeBar();
        // 动态绑定：rAF 每帧校验输入框矩形（布局/窗口变化即时重钉），撤回态结束随 cleanup 停止
        var rafId = 0;
        var syncLoop = function () { try { placeBar(); } catch (e) { /* ignore */ } rafId = requestAnimationFrame(syncLoop); };
        ...
        return function () {
          try { if (mo) mo.disconnect(); } catch (e) { /* ignore */ } try { cancelAnimationFrame(rafId); } catch (e2) { /* ignore */ }
