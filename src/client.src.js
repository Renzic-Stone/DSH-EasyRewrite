...
// ---------- 滚动锚定：已交还官方 chatScrollPositions（按会话原生保存/恢复滚动位，review #4 删自建轮子） ----------
    ...
    function EditModelPicker(props) {
      ...
      var menuStyle = { position: "absolute", bottom: "calc(100% + 8px)", left: "0", minWidth: "250px", maxWidth: "330px", maxHeight: "320px", overflowY: "auto", background: "var(--dsw-alias-bg-layer-3, rgba(42,42,46,0.98))", border: "1px solid var(--dsw-alias-border-l2, rgba(128,128,128,0.25))", borderRadius: "12px", boxShadow: "0 8px 24px rgba(0,0,0,0.35)", padding: "6px", zIndex: 60 };
      ...
      return React.createElement("div", { ref: rootRef, style: { position: "relative", display: "inline-block" } },
        React.createElement("button", {
          ...
          React.createElement("span", { style: { color: "var(--dsw-alias-label-tertiary)", fontSize: "10px", display: "inline-block", transform: menuOpen ? "rotate(180deg)" : "none", transition: "transform .15s" } }, "⌄")
        ),
