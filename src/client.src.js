  ...
  factory: function (require) {
    ...
    var SETTINGS_I18N = {
      zh: {
        ...
        attachWarning: "此消息含附件，编辑重发后附件不会保留",
      ...
      en: {
        ...
        attachWarning: "This message has attachments; they will not be preserved after editing",
      ...
      ja: {
        ...
        attachWarning: "このメッセージには添付があります。編集後の再送では添付は保持されません",
    ...
    function UserBubbleView(props) {
      ...
      // review M4：附件检测——编辑重发不会保留附件，进入编辑态时提示
      var hasAttachment = false;
      try {
        if (data && Array.isArray(data.content)) {
          for (var bi = 0; bi < data.content.length; bi++) {
            var blk = data.content[bi];
            if (blk && blk.type !== "text") { hasAttachment = true; break; }
          }
        }
      } catch (e) { /* ignore */ }
      ...
      async function confirmEdit() {
        ...
        try {
          ...
          // review M6：resume-send 带 TTL 时间戳
          try { localStorage.setItem("dsh-easyrewrite:resume-send:" + newId, JSON.stringify({ draftText: newText, t: Date.now() })); } catch (e) { /* ignore */ }
          registerVersionFork(sid, newId);
          try {
      ...
      if (editing) {
        ...
        return React.createElement(
          ...
          React.createElement(
            ...
            hasAttachment ? React.createElement("div", { style: { fontSize: "12px", color: "var(--dsw-alias-label-warning, #b7791f)", marginBottom: "6px", lineHeight: "1.5" } }, L.attachWarning) : null,
