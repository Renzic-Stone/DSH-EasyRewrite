      ...
      try {
        ...
        if (parentId) {
          if (mode === "edit" && typeof text === "string") {
            try { localStorage.setItem("dsh-easyrewrite:resume-send:" + parentId, JSON.stringify({ draftText: text, t: Date.now(), imageIds: imageIds || [] })); } catch (e) { /* ignore */ }
          }
      ...
      try {
        ...
        if (wsId && props.ctxWorkspaces && typeof props.ctxWorkspaces.connectWorkspace === "function") {
          props.ctxWorkspaces.connectWorkspace(wsId).then(function (newId) {
            if (!newId) return;
            if (mode === "edit" && typeof text === "string") {
              try { localStorage.setItem("dsh-easyrewrite:resume-send:" + newId, JSON.stringify({ draftText: text, t: Date.now(), imageIds: imageIds || [] })); } catch (e) { /* ignore */ }
            }
    ...
    function UserBubbleView(props) {
      ...
      async function confirmEdit() {
        ...
        try {
          ...
          try { localStorage.setItem("dsh-easyrewrite:resume-send:" + newId, JSON.stringify({ draftText: newText, t: Date.now(), imageIds: editImgIds })); } catch (e) { /* ignore */ }
          try {
