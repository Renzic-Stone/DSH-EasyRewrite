...
// ---------- 滚动锚定：已交还官方 chatScrollPositions（按会话原生保存/恢复滚动位，review #4 删自建轮子） ----------
    ...
    function UserBubbleView(props) {
      ...
      // 编辑态：document 捕获级接管文件拖拽（bug①修复+拖入overlay）。
      // dragenter/dragover/drop 全部捕获级拦截 → 官方全屏提示层不出现（不卡死）。
      // overlay：虚线框+毛玻璃+提示文字，灰色待机→蓝色激活，drop 后移除。随编辑态启停。
      React.useEffect(function () {
        if (!editing) return;
        var overlayEl = null;
        var dragDepth = 0;
        ...
        function ensureOverlay() {
          ...
          overlayEl.setAttribute("data-dsh-easyrewrite", "drag-overlay");
          overlayEl.style.cssText = "position:fixed;inset:12px;z-index:99998;pointer-events:none;"
            + "border:3px dashed var(--dsw-alias-label-tertiary,rgba(128,128,128,0.5));"
            + "border-radius:16px;"
          ...
          var isCompact = editWidthMode() === "compact";
          text.textContent = isCompact ? L.insertImageShort : L.dragHint;
          text.style.cssText = "font-size:15px;color:var(--dsw-alias-label-tertiary,rgba(128,128,128,0.7));"
            + "transition:color .15s;text-align:center;padding:0 20px;";
          overlayEl.appendChild(text);
          overlayEl._text = text;
          document.body.appendChild(overlayEl);
        }
        function activateOverlay() {
          if (!overlayEl) return;
          overlayEl.style.borderColor = "var(--dsw-static-deepseek-500,#4d6bfe)";
          overlayEl.style.background = "rgba(77,107,254,0.06)";
          if (overlayEl._text) overlayEl._text.style.color = "var(--dsw-static-deepseek-500,#4d6bfe)";
        }
        function removeOverlay() {
          if (overlayEl && overlayEl.parentNode) overlayEl.parentNode.removeChild(overlayEl);
          overlayEl = null; dragDepth = 0;
        }
        function onDragEnter(e) {
          if (!looksLikeFileDrag(e)) return;
          suppress(e);
          dragDepth++;
          if (dragDepth === 1) { ensureOverlay(); activateOverlay(); }
        }
