...
// ---------- 滚动锚定：已交还官方 chatScrollPositions（按会话原生保存/恢复滚动位，review #4 删自建轮子） ----------
    ...
    function RecallBanner(props) {
      ...
      React.useEffect(function () {
        ...
        label.style.cssText = "font-size:16.8px;color:var(--dsw-alias-label-secondary);line-height:26.4px;background:var(--dsw-alias-interactive-bg-hover,rgba(128,128,128,0.12));border-radius:999px;padding:1.2px 14.4px;";
        ...
        xBtn.style.cssText = "border:none;cursor:pointer;width:28.8px;height:28.8px;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;color:var(--dsw-alias-label-secondary);font-size:21.6px;line-height:21.6px;padding:0;background:var(--dsw-alias-interactive-bg-hover,rgba(128,128,128,0.12));";
        xBtn.textContent = "×";
        xBtn.addEventListener("click", function (e) {
          ...
          // 取消撤回：清空输入框内全部图片（含确认时重建的 + 用户后加的）——恢复原样
          try {
            var killIds = {};
            var snapX = typeof props.useInput === "function" ? props.useInput(function (q) { return q; }) : null;
            if (snapX && Array.isArray(snapX.imageIds)) { for (var ki = 0; ki < snapX.imageIds.length; ki++) killIds[snapX.imageIds[ki]] = true; }
            for (var kp = 0; kp < pendingAttachIds.length; kp++) killIds[pendingAttachIds[kp]] = true;
            if (ia && typeof ia.removeImage === "function") {
              for (var kk in killIds) { try { ia.removeImage(kk); } catch (e3) { /* ignore */ } }
            }
          } catch (e2) { /* ignore */ }
          pendingAttachIds = [];
          }
        ...
        if (ia && typeof ia.setDraft === "function" && typeof ia.submit === "function") {
          ...
          var finishWithImages = function (carried) {
            if (carried) doSubmit();
              ...
              log("error", "attach", "resume 图片无法恢复——已取消自动发送，草稿保留在输入框（可手动补图）");
              showRecallError(L.errGeneric);
            }
          };
          var verifyAndSend = function () {
            var snap = null;
            try { snap = typeof props.useInput === "function" ? props.useInput(function (s) { return s; }) : null; } catch (e) { /* ignore */ }
            var nowIds = snap && Array.isArray(snap.imageIds) ? snap.imageIds : [];
            var missing = savedIds.filter(function (id) { return nowIds.indexOf(id) === -1; });
            if (missing.length === 0) {
