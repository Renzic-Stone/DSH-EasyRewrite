...
// ---------- 滚动锚定：已交还官方 chatScrollPositions（按会话原生保存/恢复滚动位，review #4 删自建轮子） ----------
     ...
     */
    function resetConversation(sessionId, mode, text, props, imageIds) {
      log("info", "reset", "首条消息重置对话", { sessionId: sessionId, mode: mode });
      // v2.1.1：目标会话（父版本或空白新会话）发送前需要恢复当前模型/挡位
      var msel = props.modelSel ? props.modelSel.capture(sessionId) : null;
      writePending(sessionId, null);
    ...
    function UserBubbleView(props) {
      ...
      var sEditImgs = React.useState([]); // 气泡编辑图片工作集 [{id, url}]
      ...
      async function confirmEdit() {
        ...
        try {
          ...
          if (isFirstUserMessage(props, myKey)) {
            ...
            resetConversation(sid, "edit", newText, props, fmIds);
            return;
          }
          ...
          if (!data || !data.ok) {
            ...
            if (errCode2 === "turn-open" || errCode2 === "no-boundary") {
              ...
              resetConversation(sid, "edit", newText, props, bridgedIds);
            } else {
        ...
        React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: "6px", marginBottom: "6px" } },
          editImages.map(function(ei, eiIdx) {
            return React.createElement("div", { key: ei.id, style: { position: "relative", width: "72px", height: "72px", borderRadius: "8px", overflow: "hidden", border: "1px solid var(--dsw-alias-border-l2, rgba(128,128,128,0.2))" } },
              ...
              React.createElement("button", { type: "button", onClick: function(ev) { ev.stopPropagation(); setEditImages(function(prev) { return prev.filter(function(x) { return x.id !== ei.id; }); }); }, style: { position: "absolute", top: "2px", right: "2px", width: "18px", height: "18px", borderRadius: "50%", border: "none", background: "rgba(0,0,0,0.55)", color: "#fff", fontSize: "12px", lineHeight: "18px", textAlign: "center", cursor: "pointer", padding: "0" }, title: "移除图片" }, "\u00d7")
            );
          })
        ),
          React.createElement(
                    ...
                    if (items[pi].type && items[pi].type.indexOf("image") === 0) {
                      ...
                      if (pf) {
                        ...
                        if (pImgs && pImgs.length > 0) {
                          setEditImages(function(prev) { return prev.concat(pImgs.map(function(im) { return { id: im.id, url: im.previewUrl }; })); });
                        }
                        break;
                      }
                    }
