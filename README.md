...
| 能力 | dsh-easyrewrite | 常见撤回/编辑类插件 |
| --- | --- | --- |
| **惰性提交**（确认后才真正修改 context，中途退出零改动） | ✅ | 多数点击即生效 |
| **无痕替换**（归档 + 同名新会话顶替，感知上就是"原对话被编辑了"） | ✅ | 少见（多为 fork 出新分支） |
| **版本翻页器 < X >**（切换历史版本 + 归档交换，侧栏只留一个活动版本） | ✅ | 少数有翻页，无归档交换 |
| **草稿持久化**（按会话 + 超时自动备份 + 异常恢复） | ✅ | 基本没有 |
| **编辑重发保留图片附件** | ✅ | 基本没有 |
| **纯官方扩展点**（零源码补丁，卸载即还原） | ✅ | 部分依赖源码补丁或未合入的运行时事件 |
| 三语界面（中文 / English / 日本語） | ✅ | 多数单一语言 |
| 全部功能可按需开关 | ✅ | 少数 |
