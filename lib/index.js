  ...
  disposers.push(ctx.webServer.register({
    ...
    handler: async (req, res) => {
      try {
        ...
        // 无痕替换：新会话继承原标题（感知上仍是原对话）
        try {
          const srcSession = ctx.sessions.get(sessionId);
          const titleSnap = srcSession && ctx.sessionTitle ? ctx.sessionTitle.get(srcSession) : undefined;
          const title = titleSnap && (titleSnap.title ?? titleSnap.text) ? (titleSnap.title ?? titleSnap.text) : undefined;
          if (title && ctx.sessionTitle) {
            ctx.sessionTitle.rename(result.child, title);
            writeLog('info', 'recall', '新会话继承标题', { newId: result.child.id, title });
          }
        } catch (titleErr) {
          writeLog('warn', 'recall', '标题继承失败（不影响撤回）', { err: String(titleErr?.message ?? titleErr) });
        }
