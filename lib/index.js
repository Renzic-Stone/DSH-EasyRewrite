...
export function apply(ctx) {
  ...
  // 注册 settings namespace（rc.7+ "注册即暴露"：让「设置→插件→插件配置」能渲染我们的卡片；
  // 实际设置值走 localStorage，namespace 仅为卡片提供 key/座位）
  try {
    ctx.settings.register(settingsNamespace('dsh-easyrewrite'), z.object({}));
