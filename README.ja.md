...
- 編集下書きはセッション単位で永続化（会話切替・リフレッシュで復元）；**添付保持の再送**——画像は編集再送時に修正テキストと一緒に保持されます。
...
| 機能 | dsh-easyrewrite | 一般的な撤回・編集プラグイン |
| --- | --- | --- |
| **遅延コミット**（確認後にのみ context を変更、途中で閉じても変更なし） | ✅ | ほとんどがクリック即反映 |
| **シームレス置換**（アーカイブ + 同名セッションが引き継ぎ——"元の会話が編集された"感覚） | ✅ | 稀（多くは fork で新ブランチ） |
| **バージョンページャー < X >**（履歴バージョン切替 + アーカイブ交換——サイドバーには常に 1 つだけ） | ✅ | あるのは少数、アーカイブ交換はなし |
| **下書き永続化**（セッション単位 + タイムアウト自動バックアップ + 異常時復元） | ✅ | ほぼなし |
| **編集再送で添付（画像）保持** | ✅ | ほぼなし |
| **公式拡張ポイントのみ**（ソースパッチなし、アンインストールで完全復元） | ✅ | 一部はソースパッチや未マージのランタイムイベントに依存 |
| 3言語 UI（中文 / English / 日本語） | ✅ | 多くは単一言語 |
| 全機能を個別にオフ可能 | ✅ | 少数 |
