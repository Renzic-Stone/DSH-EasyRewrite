...
| 撤回（Recall） | ✅ | ✅ |
| **遅延コミット**（確認後にのみ context を変更、途中で閉じても変更なし） | ✅ | 多くは即時反映 |
| **シームレス置換**（アーカイブ + 同名セッションが引き継ぎ） | ✅ | 稀 |
| **バージョンページャー < X >**（履歴切替 + アーカイブ交換——常に 1 つだけ） | ✅ | 競合に同機能なし |
| **下書き永続化 + 自動バックアップ + 異常時復元** | ✅ | 競合に同機能なし |
| **編集再送で添付（画像）保持** | ✅ | 競合に同機能なし |
| **公式拡張ポイントのみ**（ソースパッチなし） | ✅ | 一部はソースパッチに依存 |
| 3言語 UI と i18n | ✅ | 多くはなし |
| 全機能を個別にオフ可能 | ✅ | 少数 |
