...
| **编辑宽度** | 设置项「**气泡框编辑宽度**」：**三个固定挡位 + 一个自定义输入框**（共四态）——① 原气泡宽度 ② **自动换行宽度（默认）**——与内容区一致（已查证：`--dsh-chat-content-width = 748px`，约 36 个全角字符）③ 输入框宽度 ④ **自定义**（手动输入像素值/宽度）；**所有挡位自带自动增高 + 内部滚动** |
...
| **气泡框编辑宽度** | 自动换行宽度 | **三个固定挡位（原气泡宽度 / 自动换行 748px / 输入框宽度）+ 自定义输入框**；选固定挡位时输入框**禁用置灰**，选「自定义」时输入框**亮起可填写**（手动宽度）；均自带自动增高+内部滚动 |
